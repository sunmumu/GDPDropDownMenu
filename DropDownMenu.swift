...
@objc public protocol DropDownMenuDelegate {
    ...
    func didChooseCell(indexPath: IndexPath, title: String)
}
...
//        contentView = UIView(frame: CGRect(origin: CGPoint.zero, size: frame.size))
        ...
        //SGP
        menuView = UITableView(frame: contentView.bounds)
...
//                self.contentView.frame.origin.y =  self.visibleContentInsets.top
	...
	@IBAction open func hide() {
		...
		UIView.animate(withDuration: 0.1,
			...
			if self.direction == .down {
                //SGP
                self.contentView.frame.origin.y = -1
...
//    }
	...
	open func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        cell.mark = cell.showsCheckmark
		
        delegate?.didChooseCell(indexPath: indexPath, title: cell.titleLabel.text!)
