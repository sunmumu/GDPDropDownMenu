...
open class DropDownTitleView : UIControl {
	...
	open lazy var menuDownImageView: UIImageView = {
		let menuDownImageView = UIImageView(image: self.imageNamed("home_arrow_cycle"))
	...
	open lazy var menuUpImageView: UIImageView = {
		let menuUpImageView = UIImageView(image: self.imageNamed("home_arrow_cycle"))
    ...
    private let downImageMargin: CGFloat = 10
...
//            titleLabel.frame.origin.x = spacing + imageView.frame.width
		...
		imageView.frame.origin.x = titleLabel.frame.maxX + downImageMargin
